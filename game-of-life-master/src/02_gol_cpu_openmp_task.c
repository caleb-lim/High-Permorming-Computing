...
void game_of_life(struct Options *opt, int *current_grid, int *next_grid, int n, int m){
    ...
    #pragma omp parallel default(none) shared(current_grid, next_grid, n, m) private(neighbours, n_i, n_j)
    {
        #pragma omp single
        {
            for(int i = 0; i < m; i++) {
                ...
                {
                    #pragma omp parallel for schedule(static) firstprivate(current_grid, next_grid, n, m) private(neighbours, n_i, n_j)
                    for(int j = 0; j < n; j++) {
                    ...
                    // #pragma omp task firstprivate(current_grid, n_i, n_j) shared(neighbours)
